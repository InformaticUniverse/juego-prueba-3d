...
## Optional VirtualJoystick reference for mobile/touch input.
## If set, movement will use the joystick instead of keyboard input.
@export var virtual_joystick: VirtualJoystick = null

## Optional VirtualJoystick reference for camera control.
## If set, camera will use the joystick instead of mouse input.
@export var camera_joystick: VirtualJoystick = null

## Optional Button or TouchScreenButton reference for jump action.
## If set, jump will use the button instead of keyboard input.
@export var jump_button: Node = null

## Optional Button or TouchScreenButton reference for sprint action.
## If set, sprint will use the button instead of keyboard input.
@export var sprint_button: Node = null
...
var jump_button_was_pressed: bool = false
...
func _ready() -> void:
	...
	# Auto-find VirtualJoystick if not explicitly set
	if not virtual_joystick:
		var parent = get_parent()
		if parent:
			# Try direct child first, then check UICanvas
			var joystick = parent.get_node_or_null("VirtualJoystick")
			if not joystick:
				joystick = parent.get_node_or_null("UICanvas/VirtualJoystick")
			if joystick:
				virtual_joystick = joystick
	
	# Auto-find CameraJoystick if not explicitly set
	if not camera_joystick:
		var parent = get_parent()
		if parent:
			# Try direct child first, then check UICanvas
			var joystick = parent.get_node_or_null("CameraJoystick")
			if not joystick:
				joystick = parent.get_node_or_null("UICanvas/CameraJoystick")
			if joystick:
				camera_joystick = joystick
	
	# Auto-find JumpButton if not explicitly set
	if not jump_button:
		var parent = get_parent()
		if parent:
			var button = parent.get_node_or_null("UICanvas/JumpButton")
			if button:
				jump_button = button
	
	# Auto-find SprintButton if not explicitly set
	if not sprint_button:
		var parent = get_parent()
		if parent:
			var button = parent.get_node_or_null("UICanvas/SprintButton")
			if button:
				sprint_button = button
...
func _handle_gravity_and_jump(delta: float) -> void:
	...
	# Use jump button if available, otherwise fall back to keyboard input
	var should_jump = false
	if jump_button:
		# Detect button press (just pressed this frame)
		var is_pressed = false
		if jump_button is TouchScreenButton:
			is_pressed = jump_button.is_pressed()
		elif jump_button.has_method("is_pressed"):
			is_pressed = jump_button.is_pressed()
		elif jump_button.has("button_pressed"):
			is_pressed = jump_button.button_pressed
		
		if is_pressed and not jump_button_was_pressed:
			should_jump = true
		jump_button_was_pressed = is_pressed
	else:
		should_jump = Input.is_action_just_pressed("jump")
	
	if not frozen and should_jump and is_on_floor():
		velocity.y = jump_velocity
		is_jumping = true

## Processes movement input and calculates movement direction relative to camera.
func _handle_movement_input() -> void:
	# Use VirtualJoystick if available, otherwise fall back to keyboard input
	if virtual_joystick:
		input_dir = virtual_joystick.get_value()
	else:
...
func _handle_controller_camera(delta: float) -> void:
	...
	# Use camera joystick if available
	if camera_joystick:
		look_dir = camera_joystick.get_value()
	...
	# Fall back to controller input if enabled
...
func _apply_movement() -> void:
	...
	# Use sprint button if available, otherwise fall back to keyboard input
	var sprint_pressed = false
	if sprint_button:
		if sprint_button is TouchScreenButton:
			sprint_pressed = sprint_button.is_pressed()
		elif sprint_button.has_method("is_pressed"):
			sprint_pressed = sprint_button.is_pressed()
		elif sprint_button.has("button_pressed"):
			sprint_pressed = sprint_button.button_pressed
	else:
		sprint_pressed = Input.is_action_pressed("sprint")
	
	is_sprinting = sprint_pressed and is_moving and not Input.is_action_pressed("walk")
	is_walking = Input.is_action_pressed("walk") and is_moving and not sprint_pressed and can_walk
...
func _unhandled_input(event: InputEvent) -> void:
	...
	# Only use mouse for camera if camera joystick is not available
	if event is InputEventMouseMotion and not camera_joystick:
